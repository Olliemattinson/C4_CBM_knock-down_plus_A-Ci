...
def save_fba_to_excel_multiple_experiments(c3_model, c4_model, D_fba, D_reactions, D_exp, CO2_levels, theNotebook,file_name='pfba_results'):
    ...
    for reaction, reaction_name in D_reactions.items():
        for CO2_level in CO2_levels:
            for exp, exp_name in D_exp.items():
                ...
                for r_obj in c3_model.reactions:
                    r_id = r_obj.id
                    r_M_flux = D_fba[reaction_name+':'+str(exp)+'_CO2:'+str(CO2_level)]['[M]_'+r_id]
                    r_B_flux = D_fba[reaction_name+':'+str(exp)+'_CO2:'+str(CO2_level)]['[B]_'+r_id]
                    r_lb = '0' if r_obj.lower_bound == 0. else '-inf' if r_obj.lower_bound == -inf else str(r_obj.lower_bound)
                    r_ub = '0' if r_obj.upper_bound == 0. else 'inf' if r_obj.upper_bound == inf else str(r_obj.upper_bound)
                    DF_fba.loc[r_id] = [r_obj.reaction,r_lb,r_ub,r_M_flux,r_B_flux]
                DF_fba.loc['[B]_RBC_h_Ex']=[c4_model.reactions.get_by_id('[B]_RBC_h_Ex').reaction,"-inf","inf","0",D_fba[reaction_name+':'+str(exp)+'_CO2:'+str(CO2_level)]['[B]_RBC_h_Ex']]
                for r_id in L_r_transport:
                    r_obj = c4_model.reactions.get_by_id(r_id)
                    r_MB_flux = D_fba[reaction_name+':'+str(exp)+'_CO2:'+str(CO2_level)][r_id]
                    r_lb = '0' if r_obj.lower_bound == 0. else '-inf' if r_obj.lower_bound == -inf else str(r_obj.lower_bound)
                    r_ub = '0' if r_obj.upper_bound == 0. else 'inf' if r_obj.upper_bound == inf else str(r_obj.upper_bound)
                    DF_fba.loc[r_id] = [r_obj.reaction,r_lb,r_ub,r_MB_flux, float('nan')]
                sheet = re.sub(r"[\[,:,*,?, \],\\,/]",'_', reaction_name+':'+str(exp)+'_CO2:'+str(CO2_level))
                DF_fba.to_excel(writer,sheet)
    writer.save()
